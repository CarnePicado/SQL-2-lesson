...
select count("s"."name") 
from "songs" "s"
inner join "albums" "a" on "s"."id"="a"."id"
where "a"."date released" between '2019-01-01' and '2020-01-01'
group by "a"."date released";
...
group by "a"."name"
having count("s"."name")=(select count("s"."name") as "c"
from "albums" "a"
inner join "songs" "s" on "a"."id"="s"."fk_albums_id"
group by "a"."name"
order by "c"
